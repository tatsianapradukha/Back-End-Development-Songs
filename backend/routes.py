...
def create_song():
    ...
    if song:
        return {"Message": f"song with id {newsong['id']} already present"}, 302
    insert_id: InsertOneResult = db.songs.insert_one(newsong)
...
def update_song(id):
    ...
    result = db.songs.update_one({"id":id}, {"$set": newsong})
